                ...
                <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="bg-white w-full max-w-sm rounded-2xl shadow-2xl p-6 relative" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="mb-6 space-y-4">
                        ...
                        {editingField.key === 'date' ? <input type="date" className="w-full p-3 border rounded-lg" value={editingField.value} onChange={(e) => setEditingField({ ...editingField, value: e.target.value })} /> : 
                         (editingField.key === 'mapUrl' || editingField.key === 'googleSheetUrl') ? <input type="text" placeholder={editingField.key === 'googleSheetUrl' ? "Dán Link Webhook Google Sheet (Của khách này) vào đây..." : "Dán link Google Maps vào đây..."} className="w-full p-3 border rounded-lg" value={editingField.value} onChange={(e) => setEditingField({ ...editingField, value: e.target.value })} /> : 
